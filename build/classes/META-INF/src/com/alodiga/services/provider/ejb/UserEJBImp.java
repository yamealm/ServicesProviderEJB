...
//            throw new EmptyListException("No distributions found");
    ...
    public List<Transaction> getInvoiceByAccontID(Long accountId) throws EmptyListException, GeneralException, NullParameterException {
        throw new UnsupportedOperationException("Not supported yet.");
    }
    ...
    public List<AccountProduct> loadAccountProductCalculationByAccountId(Long accountId) throws NullParameterException, EmptyListException, GeneralException{
     ...
     public BalanceHistory saveBalanceHistory(EJBRequest request) throws NullParameterException, GeneralException {
        return (BalanceHistory) saveEntity(request, logger, getMethodName());
    }

    public BalanceHistory loadLastBalanceHistoryByAccountId(Long accountId) throws RegisterNotFoundException, GeneralException, NullParameterException {
        if (accountId == null) {
            throw new NullParameterException(logger, sysError.format(EjbConstants.ERR_NULL_PARAMETER, this.getClass(), getMethodName(), QueryConstants.PARAM_ACCOUNT_ID), null);
        }
        BalanceHistory balanceHistory = new BalanceHistory();
        try {
            Query query = entityManager.createQuery("SELECT bh FROM BalanceHistory bh WHERE bh.account.id =?1 ORDER BY bh.id DESC");
            query.setMaxResults(1);
            query.setParameter("1", accountId);
            balanceHistory = ((List<BalanceHistory>) query.getResultList()).get(0);
        } catch (NoResultException ex) {
