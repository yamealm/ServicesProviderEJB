...
import com.alodiga.services.provider.commons.ejbs.PrepayNationTopUpUpdateTimerEJBLocal;
...
@Stateless(name = EjbConstants.PREPAY_NATION_TOP_UP_UPDATE_TIMER_EJB, mappedName = EjbConstants.PREPAY_NATION_TOP_UP_UPDATE_TIMER_EJB)
@TransactionManagement(TransactionManagementType.BEAN)
public class PrepayNationTopUpUpdateTimerEJBImp extends AbstractSPEJB implements PrepayNationTopUpUpdateTimerEJB, PrepayNationTopUpUpdateTimerEJBLocal {

    private static final Logger logger = Logger.getLogger(PrepayNationTopUpUpdateTimerEJBImp.class);
    @EJB
    ...
    private void createTimer() {
        ctx.getTimerService().createTimer(initialExpiration.getTime(), timeoutInterval, EjbConstants.PREPAY_NATION_TOP_UP_UPDATE_TIMER_EJB);
    }

    @Timeout
    public void execute(Timer timer) {
        try {
            logger.info("[PrepayNationTopUpUpdateTimerEJB] Ejecutando");
    ...
    private void executeUpdate() throws Exception {
        ...
        try {
            topUpProductEJB.executePrepayNationTopUpUpdate();
    ...
    private void setTimeoutInterval() throws Exception {
        ...
        initialExpiration.set(Calendar.MINUTE, 50);
